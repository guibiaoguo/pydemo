...
def jsontoM3U8(path,wfile):
    keylist2, bookFaillist2, key2list2, bookSuccesslist2 = jsonToList(os.path.join(path,'bookshelf'),'bookUrl','ruleBookContent.content')
    print(len(keylist2))
...
def parseBook(book):
    book1={}
    book1['bookInfoBean'] = {}
    book1['bookInfoBean']["author"]=book['author']
    book1['bookInfoBean']["bookSourceType"]=''
    book1['bookInfoBean']["chapterUrl"]=book['tocUrl']
    book1['bookInfoBean']["coverUrl"]=book.get('coverUrl')
    book1['bookInfoBean']["finalRefreshData"]=book['lastCheckTime']
    book1['bookInfoBean']["introduce"]=book.get('intro')
    book1['bookInfoBean']["name"]=book['name']
    book1['bookInfoBean']["noteUrl"]=book['bookUrl']
    book1['bookInfoBean']["origin"]=book['originName']
    book1['bookInfoBean']["tag"]=book['origin']
    book1['lastChapterName']=book.get('durChapterTitle')
    book1['noteUrl']=book['bookUrl']
    book1['tag']=book['origin']
    book1['chapterListSize']=book['durChapterIndex']
    return book1

def legadoToBook(path,wfile):
    keylist2, bookFaillist2, key2list2, bookSuccesslist2 = jsonToList(os.path.join(path,'bookshelf'),'bookUrl','ruleBookContent.content')
    print(len(keylist2))
    print(len(set(keylist2)))
    print(len(key2list2))
    ...
    print(len(bookFaillist2))
    print(len(bookSuccesslist2))
    bookshelfSet=list(map(parseBook, bookFaillist2))
    # input("开始过滤数据")
    # bookSuccessSet = list(deleteDuplicateMD5Key(bookFaillist2).values());
    # print(len(bookSuccessSet))
    # bookSuccessSetGroup = deleteDuplicateGroup(bookSuccessSet, 'bookSourceUrl')
    ...
    with open('myBookShelf.json','w',encoding='utf-8') as f:
        json.dump(bookshelfSet, f, ensure_ascii=False,sort_keys=True, indent=4, separators=(',', ':'))  

if __name__ == '__main__':
    # toM3U8("E:\\workspace\\hikerView\\tv\\202006")
    # readM3U8("C:\\Users\\bill\\Desktop\\Crack\\直播-online.m3u8")
    legadoToBook("e:\\workspace\\pydemo","fail")